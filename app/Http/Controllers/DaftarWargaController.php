...
class DaftarWargaController extends Controller
{
    ...
    public function cariwargarw(Request $request)
    {
        ...
        $role = Auth::user()->role;
        $no_rt = Profil::where('user_id', $id)->pluck('rt_id');
        $warga = DB::table('users')->where('role', '=', 6)->orWhere('role', '=', 5)->orWhere('role', '=', 4)->orWhere('role', '=', 3)->get();
        $cari = $request->searchwarga;
    ...
    public function admins()
    {
        ...
        $role = Auth::user()->role;
        $no_rt = Profil::where('user_id', $id)->pluck('rt_id');
        $warga = DB::table('users')->where('role', '=', 6)->orWhere('role', '=', 7)->orWhere('role', '=', 8)->orWhere('role', '=', 5)->orWhere('role', '=', 4)->paginate(5);
